...
float hash(float a) {
    return floor(cos(a) * seed1 + sin(a * seed2));
}

void main() {

    vec2 uv = (2.0f * gl_FragCoord.xy - RENDERSIZE.xy) / RENDERSIZE.y;
    uv -= center.xy;
    uv *= 10.5f - scale;
    float C = sin(TIME * rate) * freq1, dist = 0.0f;
    for(float i = 10.0f; i < 90.0f; i++) {
        float R = C + i;
        vec2 N = vec2(sin(R), cos(R));
        N *= abs(hash(R)) * freq2;
        dist += sin(i + loops * distance(uv, N));
    }
    gl_FragColor = vec4(vec3(dist), 1.0f);
}
