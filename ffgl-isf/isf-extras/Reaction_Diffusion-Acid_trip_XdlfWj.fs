...
vec4 tx(in vec2 p){ return IMG_NORM_PIXEL(BufferA,mod(p,1.0)); }
...
void main() {
	if (PASSINDEX == 0)	{
	    ...
	    vec2 pw = 1./RENDERSIZE.xy; // Relative pixel width. Used for neighboring pixels, etc.
	    ...
	    float ifr = min(0.97, float(FRAMEINDEX) / 100.0);
	    gl_FragColor.xy = mix(IMG_NORM_PIXEL(inputImage,mod(uv,1.0)).xz, clamp(vec2(newReactDiff, avgReactDiff/.98), 0., 1.), ifr);
    ...
    // Done.
    isf_FragColor = vec4(min(col, 1.), 1.); 
