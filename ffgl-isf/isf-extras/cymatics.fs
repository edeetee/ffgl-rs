...
#define S(X, Y, period) gl_FragColor += sin(length(uv + vec2(X,Y)*d2)*zoom)-.2
...
void main() {
    ...
    d2 = 8.0f - 2.0f * sin(5.0f + progress * PI * 2) + offset * 10, //number to move 5 wave machines outwards
    d1 = .5f;
    ;// wave width

    uv = .5f * (uv - vec2(d4, d3)); //move pic around
